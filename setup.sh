...
# ── Create ACCOUNTS symlink at project root ────────────────
# This makes ACCOUNTS visible when you open the project in Claude Code / OpenCode
ACCOUNTS_LINK="$(pwd)/ACCOUNTS"
if [ ! -e "$ACCOUNTS_LINK" ]; then
  ln -sf "$JARVIS_HOME/ACCOUNTS" "$ACCOUNTS_LINK"
  ok "  ✓ ACCOUNTS → $JARVIS_HOME/ACCOUNTS (symlink created)"
  # Ensure symlink is gitignored
  if ! grep -qx "/ACCOUNTS" .gitignore 2>/dev/null; then
    echo "/ACCOUNTS" >> .gitignore
  fi
else
  ok "  ✓ ACCOUNTS symlink already exists"
fi

# ── Create ACCOUNTS/.claude/CLAUDE.md for OpenCode context ─
ACCOUNTS_CLAUDE_DIR="$JARVIS_HOME/ACCOUNTS/.claude"
mkdir -p "$ACCOUNTS_CLAUDE_DIR"
cat > "$ACCOUNTS_CLAUDE_DIR/CLAUDE.md" << 'CLAUDEMD'
# ACCOUNTS — JARVIS Deal Intelligence

This folder contains all your account and opportunity data.
Each subfolder is one account or opportunity — create them freely, JARVIS treats them all the same.
...
MCP tools are registered globally — you can use them from this workspace too.

**Quick commands:**
  • "list my accounts"
  • "show my pipeline"
  • "brief me on [account name]"
  • "prep me for meeting with [account name]"
CLAUDEMD
ok "  ✓ ACCOUNTS/.claude/CLAUDE.md created"
